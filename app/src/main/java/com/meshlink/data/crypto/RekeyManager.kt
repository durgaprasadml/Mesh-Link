...
class RekeyManager @Inject constructor(
    ...
    // Callbacks to BleRepository (to avoid circular dependency loop)
            ...
            while (true) {
                ...
                for (peerId in sessionManager.getAllSessionPeers()) {
                    ...
                    if (session.previousKeyVersion != 0 && now > session.rekeyTimestamp + 60_000) {
                        Log.d(TAG, "Grace period ended for peer $peerId, deleting previous AES key")
                    ...
                    if (pending != null && now > pending.timestamp + 5000) {
                        if (pending.retries < 3) {
                            pending.retries++
                            Log.w(TAG, "Rekey timed out for $peerId. Retrying (${pending.retries}/3)...")
                            sendRekeyPacket(peerId, session.sessionId, session.keyVersion, pending.nextKeyVersion, pending.myEphemeralPublicKeyBase64)
                        } else {
                            Log.e(TAG, "Rekey failed 3 times for $peerId. Falling back to full handshake.")
    ...
    private fun initiateRekey(peerId: String, reason: String) {
        ...
        Log.d(TAG, "Initiating secure rekey for $peerId (Reason: $reason)")
        ...
        scope.launch {
            try {
                ...
                val packet = MeshPacket(
                    senderId = "", // Filled by BleRepository
                    targetId = peerId,
                    payload = payload,
                ...
                Log.e(TAG, "Failed to send rekey packet: ${e.message}")
            }
        }
    }
    ...
    fun handleRekeyPacket(peerId: String, payload: String, senderPublicKey: String?) {
        try {
            ...
            if (session == null || session.sessionId != sessionId) {
                Log.e(TAG, "Rekey failed: Session mismatch")
                return
            }
            ...
            if (signingKey == null || !cryptoManager.verifySignature(signingKey, dataToVerify, sigBytes)) {
                Log.e(TAG, "Rekey failed: Invalid signature")
                return
            }
            ...
            if (pending != null && pending.nextKeyVersion == nextKv) {
                ...
                Log.d(TAG, "✅ Secure rekey successful (Initiator). New Key Version: $nextKv")
            } else {
                ...
                Log.d(TAG, "✅ Secure rekey successful (Responder). New Key Version: $nextKv")
            ...
            Log.e(TAG, "Failed to handle rekey packet: ${e.message}")
        }
    }
