...
class WifiSocketTransport @Inject constructor() {
        ...
        scope.launch {
            try {
                ...
                while (isActive) {
                    ...
                    MeshLogger.d(TAG, "Client connected: ${client.inetAddress.hostAddress}")
    ...
    fun connectAsClient(hostAddress: String) {
        scope.launch {
            try {
                ...
                MeshLogger.d(TAG, "Connected to Group Owner!")
        ...
        try {
            ...
            listenJob = scope.launch {
                try {
                    ...
                    MeshLogger.e(TAG, "Socket read error: ${e.message}")
                } finally {
                    disconnect()
                }
            }
    ...
    fun disconnect() {
        try {
            ...
            reader = null
            activeSocket = null
            MeshLogger.d(TAG, "Socket disconnected")
        } catch (e: Exception) {
            MeshLogger.e(TAG, "Error during disconnect: ${e.message}")
        }
    }
